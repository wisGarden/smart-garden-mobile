...
export class InsectSearchPage {
    ...
    loadDiseaseData(isFirstTime) {
        this.network.getInsectList(this.keyword, this.currentPage, (data, error) => {
            if (data) {
                if (data.data != null) {
                    if (isFirstTime) {
                        this.insectList = data;
                    } else {
                        for (let i = 0; i < data.data.length; i++) {
                            this.insectList.data.push(data.data[i]);
                        }
                    }
                } else {
                    if (isFirstTime) {
                        ...
                        this.hasMoreInsect = false;
                        if (this.infiniteScroll != null && !this.hasMoreInsect && !this.hasMorePlant) {
    ...
    loadHotData(isFirstTime) {
        ...
        this.network.getInsectList('', this.currentPage++, (data, error) => {
            if (data) {
                if (data.data != null) {
                    if (isFirstTime) {
                        this.hotList = data;
    ...
    shouldLoadHotData(isFirstTime) {
        if (this.insectList == null && this.plantList == null) {
            this.loadHotData(isFirstTime);
            return true;
        }
        if (this.insectList != null && this.insectList.data == null && this.plantList == null) {
            this.loadHotData(isFirstTime);
            return true;
        }
        if (this.plantList != null && this.plantList.data == null && this.insectList == null) {
            this.loadHotData(isFirstTime);
            return true;
        }
        if (this.insectList != null && this.insectList.data == null && this.plantList != null && this.plantList.data == null) {
            this.loadHotData(isFirstTime);
            return true;
        }
        return false;
    ...
    setHistory(keyword) {
        ...
        this.sqlite.create({
            ...
            db.executeSql('SELECT * FROM keywords WHERE keyword = ? and delete_time = 0 and type = 1', [keyword])
                .then(res => {
                    if (res.rows.length != 0) {
                        ...
                        let updateTime = Date.now() / 1000;
                        db.executeSql('INSERT INTO keywords VALUES(NULL, 1, ?, ?, 0)',
                            [keyword, updateTime])
    ...
    loadHistory() {
        this.sqlite.create({
            ...
            db.executeSql('CREATE TABLE IF NOT EXISTS keywords(id INTEGER PRIMARY KEY, type INT, keyword TEXT DEFAULT 0, update_time INT DEFAULT 0, delete_time INT DEFAULT 0)', {})
                .then(res => console.log('Executed SQL'))
                .catch(e => console.log(e));
            db.executeSql('SELECT * FROM keywords WHERE type = 1 and delete_time = 0 ORDER BY update_time DESC limit 15', {})
                .then(res => {
                    this.historyList = [];
